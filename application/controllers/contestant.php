...
class contestant extends CI_Controller{
	...
	public function forgot_password(){
		...
		if (ISSET($_POST['email']))
		{
			...
			if ($result>0)
			{
				...
				{
					$isi['message']='Maaf, ada masalah dengan server, pesan tidak dapat dikirim ke alamat email anda.';
					$isi['isi']='error';
					$this->load->view('template',$isi);
				}
			}
	...
	public function register_jpc(){
		...
		if (ISSET($_SESSION['contestant_id']))
		{
			$isi['message']='Maaf, Anda tidak dapat mengakses halaman ini.';
			$isi['isi']='error';
			$this->load->view('template',$isi);
		}
		else
			...
			if ((ISSET($_POST["recaptcha_challenge_field"]))&&(ISSET($_POST["recaptcha_response_field"])))
			{
				...
				if (!$resp->is_valid) {
						...
						(ISSET($_POST['alamat_sekolah']))&&(ISSET($_POST['nama_pembimbing'])))
					{
						...
						$result = $this->co->check_duplicate_jpc($_POST['nama_lengkap'], $_POST['nomor_ponsel'], $_POST['alamat'], $_POST['email'], 
																		$_POST['kelas'], $_POST['nama_sekolah'], $_POST['alamat_sekolah'], $_POST['nama_pembimbing']);
						if (!(empty($result)))
							array_push($error,"Data-data yang dimasukkan sudah pernah terdaftar.");
						...
						{			
							...
							if ($result != null)
							{
								...
								$isi['message']='Selamat anda telah berhasil registrasi ITB Junior Programming Contest.<br />'.
												'Berikut data username dan password Anda:<br /><br />'.
												'username: '.$result['username'].'<br />password: '.$result['password'].'<br /><br />Selamat bertanding dalam ITB Junior Programming Contest 2012.';
								$this->load->view('template',$isi);
							}
							else
							{
			...
			if ((ISSET($_POST["recaptcha_challenge_field"]))&&(ISSET($_POST["recaptcha_response_field"])))
			{
				...
				if (!$resp->is_valid) {
					...
					(ISSET($_POST['nama_anggota_satu']))&&(ISSET($_POST['ponsel_anggota_satu']))&&(ISSET($_POST['email_anggota_satu'])))
					{
						...
						$result = $this->co->check_duplicate_spc($_POST['nama_tim'], $_POST['nama_universitas'], $_POST['alamat_universitas'], $_POST['nama_anggota_satu'], 
						...
						{			
							...
							if ($result != null)
							{
								...
								$isi['message']='Selamat anda telah berhasil registrasi ITB Senior Programming Contest.<br />'.
												'Berikut data username dan password Anda:<br /><br />'.
												'username: '.$result['username'].'<br />password: '.$result['password'].'<br /><br />Selamat bertanding dalam ITB Senior Programming Contest 2012.';
								$this->load->view('template',$isi);
							}
						}
					}
